...
#        print('Current line:', line)
    ...
    return None
############################################################################
def find_element2(section, index):
    lines = section.split('\n')
    for line in lines:
        print('Current line:', line)
        if line.startswith('O ') or line.startswith('S '):
            return index
    return None
#########################################################################
...
for file_forstruct_path in file_forstruct_paths:
    sections = split_file_by_keyword(file_forstruct_path, keyword1)
    for index, section in enumerate(sections):
        found_index_O = find_element2(section, index)
        if found_index_O is not None:
            O_indices.append(found_index_O)
...
def process_section2(section):
    ...
    for line in lines:
        if re.match(r'^ \d',line):
            numbers = re.findall(r'-?\d+\.\d+', line)
            if kk == 2 and line.startswith(' 1'):
                line = re.sub(r'\d+\.\d+', '0.000', line, count=2)
                line = re.sub(r'\d+\.\d+', '3.30', line, count=1)
            elif kk == 3 and line.startswith(' 2'):
                line = re.sub(r'\d+\.\d+', '0.000', line, count=2)
                line = re.sub(r'\d+\.\d+', '3.30', line, count=1)
            elif kk == 4 and line.startswith(' 2'):
                line = re.sub(r'\d+\.\d+', '0.000', line, count=2)
                line = re.sub(r'\d+\.\d+', '3.30', line, count=1)
            else:
                line = re.sub(r'\d+\.\d+', '0.000', line, count=2)
...
def process_section3(section):
    ...
    for line in lines:
        if re.match(r'^ \d',line):
            numbers = re.findall(r'-?\d+\.\d+', line)
#            print(numbers)
            if len(numbers) > 1 and line.startswith(' 1'):
                line = re.sub(r'\d+\.\d+', '0.003', line, count=2)
                line = re.sub(r'\d+\.\d+', '0.30', line, count=1)
            else:
                line = re.sub(r'\d+\.\d+', '0.000', line, count=2)
...
for file_path in file_paths:
    ...
    for index, section in enumerate(sections):
        ...
        elif index in O_indices:
            processed_section = process_section3(section)
            processed_sections.append(processed_section)
        else:
